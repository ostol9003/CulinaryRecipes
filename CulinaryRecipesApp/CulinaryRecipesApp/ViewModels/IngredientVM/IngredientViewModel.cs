...
namespace CulinaryRecipesApp.ViewModels.IngredientVM
{
    public class IngredientViewModel : AItemsViewModel<IngredientDto>
    {
        ...
        public override async Task GoToAddPage() => await Shell.Current.GoToAsync(nameof(IngredientNewPage));

        public override async Task GoToDetailsPage(IngredientDto Ingredient)
        => await Shell.Current.GoToAsync($"{nameof(IngredientDetailPage)}?{nameof(IngredientDetailsViewModel.ItemId)}={Ingredient.Id}");
    }
}
